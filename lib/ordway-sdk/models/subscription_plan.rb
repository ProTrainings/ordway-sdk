...
module Ordway
  class SubscriptionPlan
    ...
    def self.base_plan_to_subscription_plan(data)
      plan = Plan.map(data)
      return if plan.charges.blank?

      plan.charges.map do |p|
    ...
    def self.map(data)
      if data.is_a?(Hash)
        return map_subscription_plan(data)
      end

      data.map do |subscription_plan_data|
        map_subscription_plan(subscription_plan_data)
      end
    end

    def self.map_subscription_plan(data)
